...
- **Project Structure Cleanup**
  ...
  - Moved `add_manifests_to_queue.py` to `scripts/` folder
  - Added `pre_alert_config.json` to `.gitignore` (user-specific settings)

### Fixed
- Removed accidental `nul` junk file from repository

### Documentation
- Updated About dialog with current feature list
- Added Pre-Alert Email Automation section to README
- Updated project structure to reflect all current directories

## [1.4.1] - 2026-02-02

### Added
- **United Business SPL ETOE Carrier Support**
  ...
  - Simple single-row-per-country structure (rows 6-33)
  - Template: `UBL_CP_Pre_Alert_SPL-ETOE.xlsx`
  - Auto-detects from B3 containing "SPL"
  - Country mapping for Korea, North Macedonia, Taiwan variations
